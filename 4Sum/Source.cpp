...
public:
	...
	vector<vector<int> > twoSum(int left, int right, int target)
	{
		...
		while(left<right)
		{
			visited.insert(data[left]);
			visited.insert(data[right]);
			if(data[left]+data[right]==target)
			{
				...
				if(data[left]+data[right]>target) right--;
					...
					left++;
			while(left < data.size() && visited.find(data[left]) != visited.end()) left++;
			while(right > 0 && visited.find(data[right]) != visited.end()) right--;
		}
